...
public class IssueMembershipCredentialTest {
    ...
    void setUp(ServiceExtensionContext context, ObjectFactory factory) {
        ...
        configMap.put("edc.registration.membership.issuance.enabled", "true");
    ...
    void shouldNotRequest_whenCredentialAlreadyIssued() {
        var resJson = """
                [
                    {"id":"credential-1","type":"MembershipCredential"}
                ]
                """;
    ...
    void shouldIssueCredential_whenNotPresent() {
        ...
        server.stubFor(get(urlPathEqualTo(credentialPath))
                .withQueryParam("type", equalTo(CREDENTIAL_TYPE))
                .inScenario("Member Credential Issuance")
        ...
        server.verify(1, postRequestedFor(urlPathEqualTo(requestPath)).withHeader("x-api-key", equalTo(SUPER_USER_API_KEY))
                ...
                .withRequestBody(matchingJsonPath("$.credentials[0].id", equalTo("membership-credential-def-1")))
                .withRequestBody(matchingJsonPath("$.credentials[0].type", equalTo("MembershipCredential")))
    ...
    void shouldRetryIssuance_whenRequestFails() {
        ...
        verify(monitor).warning("Membership credential issuance failed.");
    }
    ...
    void shouldThrow_whenCredentialIssuanceFails() {
        ...
        assertThatThrownBy(() -> extension.start())
                .isInstanceOf(EdcException.class)
                .hasMessageContaining("Could not start membership credential request.");
    ...
    void shouldThrow_whenCheckForCredentialAfterNotPresentFails() {
        ...
        assertThatThrownBy(() -> extension.start())
                ...
                .cause().hasMessageContaining("Could not query credentials of type: %s".formatted(CREDENTIAL_TYPE));
    ...
    @Test
    void shouldWarnAndNotThrow_whenIssuanceReturns4xx() {
        server.stubFor(get(urlPathEqualTo(credentialPath))
                .withQueryParam("type", equalTo(CREDENTIAL_TYPE))
                .willReturn(okJson("[]")));
        ...
        verify(monitor).warning("Membership credential request failed with error: 400 - Bad Request.");
    }
